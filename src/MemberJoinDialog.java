...
public class MemberJoinDialog extends JDialog implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getActionCommand().equals(Const.OK)) {
			...
			else if(pwdTextField.getPassword().length == 0) {
			...
			else
			{
				...
				//없으면
				if(Database.getInstance().checkExistName(nameTextField.getText()) == false) {
					/* 데이터베이스에 값을 입력합니다. */
				...
				//있으면 경고 표시
					...
					JOptionPane.showMessageDialog(null, "중복되는 name 입니다.");
				}
				dispose();	
			}
		}
